...
function PostsList() {
  ...
  const handleCommentClick = (postId) => {
    setData((prevData) =>
      prevData.map((post) =>
        post._id === postId ? { ...post, commentChecked: !post.commentChecked } : post
      )
    );
  };
        ...
        <Container key={post._id} className='postCard'>
          ...
          <Row className='likes_Comments_Counter'>
            <Col className='likesCounter'>
              <FontAwesomeIcon icon={solidThumbsUp} className='likeIconCounter' />{' '}
              15
            </Col>
          ...
          <Row className='likes_Comments_Save'>
            <Col xs={4} className='d-flex LCS'>
              <Button
                className={`likeButton ${post.likeChecked ? 'likeChecked' : ''}`}
            ...
            <Col xs={4} className='d-flex LCS'>
              <Button
                className={`commentButton ${post.commentChecked ? 'commentChecked' : ''}`}
                onClick={() => handleCommentClick(post._id)}
              >
                <FontAwesomeIcon
                  icon={faComment}
                  className={`commentIconAction ${post.commentChecked ? 'commentChecked' : ''}`}
                />
                <span className={`ml-6 commentButtonText ${post.commentChecked ? 'commentChecked' : ''}`}>
                  Comment
                </span>
              </Button>
            </Col>
            <Col xs={4} className='d-flex LCS'>
              <Button
                className={`saveButton ${post.saveChecked ? 'saveChecked' : ''}`}
                onClick={() => handleSaveClick(post._id)}
              >
                <FontAwesomeIcon
                  icon={post.saveChecked?solidStar:outlineStar}
                  className={`saveIconAction ${post.saveChecked ? 'saveChecked' : ''}`}
                />
                <span className={`ml-6 saveButtonText ${post.saveChecked ? 'saveChecked' : ''}`}>
                  Save
                </span>
              </Button>
            </Col>
